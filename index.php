<?php
@include_once("config.php");

if (isset($_POST['insert'])) {

    $target = "user-img/" . basename($_FILES['img']['name']);

    $image = $_FILES['img']['name'];
    $fullname = $_POST['fullname'];

    if ($image ==="" || $fullname ==="")  {
        $Mwg = "Please try your data.";
        header("refresh:5;");
    }else {
        $insertData = mysqli_query($conn,"INSERT INTO user set fullname='$fullname', img='$image'" );
        if (move_uploaded_file($_FILES['img']['tmp_name'], $target)) {

            $Msg = "Succesfully data saved";
            header("refresh:5;"); 

        }else {
            $Meg = "Warning!. try your again";
            header("refresh:5;");
        }
    }

}

 ?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Upload Image to database</title>
    <!-- custom tailwindcss  -->
    <script src="https://cdn.tailwindcss.com"></script>
    <!-- custom font icons  -->
    <link rel="stylesheet"
        href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200" />
</head>

<body>
    <div class="w-full flex justify-center gap-2 relative">

        <?php
            if (isset($Mwg)) {
                ?>
                <div
                    class="w-full h-screen absolute z-40 flex justify-center items-center gap-4 text-xl text-center bg-[#29292981]">
                    <div
                        class="sm:w-[30rem] relative flex justify-center items-center flex-col gap-5 w-full h-[16rem] bg-white rounded-md">
                        <span class="material-symbols-outlined text-[5rem] text-yellow-400">question_mark</span>
                        <span
                            class="text-md absolute bottom-0 left-0 rounded-md rounded-br-md bg-yellow-400 w-full py-4 px-8 shadow-md font-semibold"><?php echo $Mwg; ?></span>
                    </div>
                </div>
                <?php
            }
        ?>
        <?php
            if (isset($Msg)) {
                ?>
                <div
                    class="w-full h-screen absolute z-40 flex justify-center items-center gap-4 text-xl text-center bg-[#29292981]">
                    <div
                        class="sm:w-[30rem] relative flex justify-center items-center flex-col gap-5 w-full h-[16rem] bg-white rounded-md">
                        <span class="material-symbols-outlined text-[5rem] text-green-500">done_all</span>
                        <span
                            class="text-md absolute bottom-0 left-0 rounded-md rounded-br-md bg-green-500 w-full py-4 px-8 shadow-md font-semibold"><?php echo $Msg; ?></span>
                    </div>
                </div>
                <?php
            }
        ?>
        <?php
            if (isset($Meg)) {
                ?>
                <div
                    class="w-full h-screen absolute z-40 flex justify-center items-center gap-4 text-xl text-center bg-[#29292981]">
                    <div
                        class="sm:w-[30rem] relative flex justify-center items-center flex-col gap-5 w-full h-[16rem] bg-white rounded-md">
                        <span class="material-symbols-outlined text-[5rem] text-rose-500">dangerous</span>
                        <span
                            class="text-md absolute bottom-0 left-0 rounded-md rounded-br-md bg-rose-500 w-full py-4 px-8 shadow-md font-semibold"><?php echo $Meg; ?></span>
                    </div>
                </div>
                <?php
            }
        ?>

        <div class="w-[40rem] h-auto rounded-md border-2 drop-shadow-md p-4">
            <div class="py-6 bg-sky-600 rounded-md text-center mb-5">
                <h2 class="text-white text-xl">Form Upload Images</h2>
            </div>
            <form action="index.php" method="POST" enctype="multipart/form-data">
                <label for="title" class=" relative block">
                    <p class="text-slate-400 font-medium my-2 text-go">Fullname</p>
                    <input type="text"
                        class=" placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-3 pl-2 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 text-sm"
                        name="fullname">
                </label>
                <label for="title" class=" relative block mt-4">
                    <p class="text-slate-400 font-medium my-2 text-go">Select images</p>
                    <input type="file"
                        class=" placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-3 pl-2 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 text-sm"
                        name="img" accept="image/png, image/jpg, image/jpeg, image/pdf">
                </label>
                <div class="mt-6 flex justify-center items-center gap-3">
                    <button type="resect" class="py-2 px-4 rounded-md bg-rose-500 text-white">refresh</button>
                    <button type="submit" class="py-2 px-4 rounded-md bg-emerald-500 text-white"
                        name="insert">insert</button>
                </div>
            </form>
        </div>

        <div class="w-full h-auto rounded-md border p-8 text-center">
            <h3 class="text-gray-700 text-2xl font-semibold m-4">All image lists</h3>
            <hr>
            <?php
            $select = mysqli_query($conn, "SELECT * FROM user");
            while($row = mysqli_fetch_array($select)) {
                ?>
                <div class="w-full relative flex justify-start items-center gap-8 my-4">
                    <img src="user-img/<?php echo $row['img'] ?>" alt="userprofile" class="w-[7.5rem] h-[7.5rem] drop-shadow-md object-cover rounded-md">
                    <h4 class="text-xl font-bold text-slate-500"><?php echo $row['fullname'] ?></h4>
                    <div class=" absolute right-3 top-8 flex justify-center items-center gap-5">
                        <a href="update.php?id=<?php echo $row['id'] ?>" class="text-xl py-2 px-6 rounded-sm bg-sky-500 text-white hover:scale-110 hover:ease-in hover:duration-300">Update</a>
                        <a href="delete.php?id=<?php echo $row['id'] ?>" class="text-xl py-2 px-6 rounded-sm bg-rose-500 text-white hover:scale-110 hover:ease-in hover:duration-300">Delete</a>
                    </div>
                </div>
                <?php
            }
            ?>
        </div>
    </div>

</body>

</html>